...
%
%       SpO2 vector of oxygenation data , length of EEG
%
%       O2rate sampling rate of O2 data
%
%  Outputs:
%   
...
if(isfield(stageData, 'eventMat'))
    ...
    if or(ismember( 'Apnea', LUT(eventToPlot,2)), ismember( 'Hypopnea', LUT(eventToPlot,2)))
        ...
        LUT = [LUT; {'' 'A/H Associated w/ an Arousal'}];
        eventToPlot = [eventToPlot size(LUT,1)];
    ...
    for i = 1:length(eventToPlot)
        ...
        elseif strcmp(LUT{eventToPlot(i), 2}, 'A/H Associated w/ an Arousal')
              ev = nansum(stageData.eventMat(:,or(strcmp(LUT(:,2), 'Apnea'), strcmp(LUT(:,2), 'Hypopnea'))),2);
              ev = [ev ~isnan(stageData.eventMat(:, or(strcmp(LUT(:,2), 'Transient Arousal'),strcmp(LUT(:,2), 'Movement Arousal'))))];
              ev = ev(:,1).*ev(:,2);
        else
...
%% Report Desaturation IF O2 Sat channel is present

% Process SpO2 data
SpO2(1:((sleepLat-1)*win*O2rate))=nan;
SpO2(sleepEnd*win*O2rate+1:end)=nan;
meanSpO2 = nanmean(SpO2(SpO2~=0));
lowSpO2 = min(SpO2(SpO2~=0));
timeBelow90 = nansum(and(SpO2>0,SpO2<90))./O2rate/60;
timeBad = nansum(SpO2==0)./O2rate/60;


report = [report, '<br><hr><h2>O<sub>2</sub> Saturation</h2>'];
report = [report, '<br><table cellpadding="5"><tr>'];
report = [report, sprintf('<tr><td><b>Mean SpO<sub>2</sub> (%%):</b></td><td></td><td></td><td>%.1f</td></tr>\n', meanSpO2)];
report = [report, sprintf('<tr><td><b>Lowest SpO<sub>2</sub> (%%):</b></td><td></td><td></td><td>%.1f</td></tr>\n', lowSpO2)];
report = [report, sprintf('<tr><td><b>SpO<sub>2</sub> < 90%% (min):</b></td><td></td><td></td><td>%.1f</td></tr>\n',  timeBelow90)];
report = [report, sprintf('<tr><td><b>SpO<sub>2</sub> off [0%%] (min):</b></td><td></td><td></td><td>%.1f</td></tr>\n',  timeBad)];
