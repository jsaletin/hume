...
%
%   This data takes a .txt 'Events' report generated by profusion using the
%   custom written Events.txt report template. An associated hypnogram file
...
elseif nargin==0
    stageData={'UniSA *.txt','.txt','convertScores_ProfusionCSR'};
...
if ~exist([path,'/',fileName,'_hyp.txt'])
    ...
    eventFile = file;
    hypFile = [path,'/',fileName,'_hyp.txt'];
end
...
rstart = textscan(fid,'%s%s%s[^\n\r]','delimiter','\t');
rdate =  textscan(fid,'%s%s%s[^\n\r]','delimiter','\t');
elength = textscan(fid,'%s%s%s[^\n\r]','delimiter','\t');

eventimport = textscan(fid,'%s%s%s','delimiter','\t');

fclose(fid);

stageData.recStart = datenum(rstart{2},'HH:MM:SS'); % PULL FROM HEADER

stageData.win = str2double(elength{2});

stageData.Notes = '';

eventsCell = cell(size(eventimport{1},1),3);

for r = 1:size(eventsCell,1)
    
    for c = 1:size(eventsCell,2)
    eventsCell{r,c} = eventimport{c}{r};
    
    if c == 2
        str = eventsCell{r,c};
        str = str(7:end);
        eventsCell{r,c} = str2num(str);
    end
...
% epLightsOff = 14;
% epLightsOn = 1093;
% stageData.lightsOFF = datenum('22:30:25','HH:MM:SS');
% stageData.lightsON = datenum('7:30:03','HH:MM:SS');

for i = 1:size(eventsCell,1)
    
        % FIND LIGHTS OFF
        if strcmpi(eventsCell{i,3},'Lights Off')
           ...
           lightsOffFound = 1;
           epLightsOff = eventsCell{i,2};
           stageData.lightsOFF = datenum(eventsCell{i,1},'HH:MM:SS');
 
           
         % FIND LIGHTS ON   
         elseif strcmpi(eventsCell{i,3},'Lights On')
           if lightsOnFound==1
                 warning('Found Duplicate Lights On. Last Is Used, Check Scored File to Confirm');
           end
            
            lightsOnFound = 1;
            epLightsOn = eventsCell{i,2};
           stageData.lightsON = datenum(eventsCell{i,1},'HH:MM:SS');
...
% FIND OTHER EVENTS
        ...
        separated = strsplit(eventsCell{i,3},', ');

        for ev = 1:size(LUT,1);
                    if sum(ismember(separated,LUT{ev,1}))>=1
                        stageData.eventMat((eventsCell{i,2}),ev)=sum(ismember(separated,LUT{ev,1}));
                    end
        end
end
